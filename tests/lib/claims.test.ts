...
const { claimAutoConfirmAt, CLAIM_AUTO_CONFIRM_HOURS } = await import("@/lib/claims");

describe("claimAutoConfirmAt", () => {
  it("is a 24-hour window", () => {
    expect(CLAIM_AUTO_CONFIRM_HOURS).toBe(24);
  });

  it("returns an ISO timestamp 24h after the given time", () => {
    const from = new Date("2026-06-03T00:00:00.000Z");
    const result = claimAutoConfirmAt(from);
    expect(result).toBe("2026-06-04T00:00:00.000Z");
  });

  it("defaults to 24h from now", () => {
    const before = Date.now();
    const result = new Date(claimAutoConfirmAt()).getTime();

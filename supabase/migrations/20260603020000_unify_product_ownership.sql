-- ============================================================
-- Unify ownership across a product's tag group.
--
-- Several tags can be linked to ONE product (multiple chips on the same
-- physical item). Ownership therefore belongs to the *product*: a claim or
-- transfer made via any tag must reflect on ALL sibling tags.
--
-- The ownership_records row stays anchored to the entry tag; reads resolve the
-- chain by product (all sibling tags). On the write side, the two ownership
-- RPCs now flip the status of EVERY tag sharing the product, not just one.
-- ============================================================
